...
uniform int dimFin_location;
uniform vec3 culoriZona_location[9];
uniform vec3 pozitiiCuloriZona_location[9];
...
uniform int luminaPodea_location;
uniform int luminaSpot_location;
...
vec3 PointLightContribution(vec3 lightPos, vec3 lightColor)
{
    ...
    color = att * (specular_light + diffuse_light);
...
void main()
{
    ...
    vec3 auxiliarPunctColorat = vec3(0, 0, 0);
    vec3 auxiliarSpotColorat = vec3(0, 0, 0);

    if (luminaPodea_location > 0) {
        for (int i = 0; i < dimFin_location; i++) {
            auxiliarPunctColorat += PointLightContribution(pozitiiCuloriZona_location[i], culoriZona_location[i]);
        }
    }
    
    if (luminaSpot_location > 0) {
         for (int i = 0; i < 4; i++) {
   ...
   // light_dir  este directia de iluminare
    ...
    out_color = vec4(color + auxiliarPunctColorat + auxiliarSpotColorat + auxiliarCuloareGlob, 1);
}
