...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-3, 0.5f, 6));
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, mapTextures["bamboo"], mapTextures["grass"], true);
    }
}
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, bool mix)
{
    ...
    // bool pt mixarea culorilor:
    GLint is_mixed = glGetUniformLocation(shader->program, "mix");
    glUniform1i(is_mixed, mix);
