...
void main()
{
    ...
    vec3 worldNorm = normalize(mat3 (Model) * v_normal);
    ...
    float diffuse_light = material_kd * max(dot(worldNorm, L), 0);
    ...
    if (diffuse_light > 0) {
    // versiune cu vector median
        specular_light = material_ks * pow(max(dot(worldNorm, H), 0), material_shininess);
    }
