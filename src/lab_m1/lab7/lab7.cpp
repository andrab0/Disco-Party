...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint shininessProp = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(shininessProp, materialShininess);

    GLint kdProp = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(kdProp, materialKd);

    GLint ksProp = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(ksProp, materialKs);

    GLint objCol = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(objCol, 1, glm::value_ptr(color));
