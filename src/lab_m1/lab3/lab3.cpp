...
void Lab3::Init()
{
    ...
    cx = (squareSide / 2) + corner[0];
    cy = (squareSide / 2) + corner[1];
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices;
           ...
           for (int i = 1; i <= 100; i++) {
               if (i < 98)
                vertices.push_back(VertexFormat(glm::vec3(acos(i * 6.28 / 100), acos((i + 1) * 6.28 / 100), acos((i + 2) * 6.28 / 100))));
            }
...
void Lab3::Update(float deltaTimeSeconds)
{
    ...
    if (flagT) {
        translateX += deltaTimeSeconds * 25;
    ...
    modelMatrix = glm::mat3(1);
    RenderMesh2D(meshes["cerc"], shaders["VertexNormal"], modelMatrix);
